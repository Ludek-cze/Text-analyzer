...
uppercase_words = len([word for word in vybrany_text.split() if word.isupper() and word.isalpha()])
uppercase_words_list = [word for word in vybrany_text.split() if word.isupper() and word.isalpha()] # Nová proměnná pro výpis slov
print(f"Slova psaná velkými písmeny: {uppercase_words_list}") # Výpis slov
...
def create_text_bar_chart(word_lengths):
    ...
    # Nastavení pevné šířky sloupce
    column_width = 12

    max_len_str = str(max_length)
    max_len_digits = len(max_len_str)

    print("\nČetnost délek slov:")  # HLAVIČKA GRAFU
    print("=" * 40)  # HLAVIČKA GRAFU
    # Změna formátování pro zarovnání "Počet"
    print(f"{'Délka':{max_len_digits+1}} {'Počet':>15}")  # HLAVIČKA GRAFU
    print("=" * 40)  # HLAVIČKA GRAFU

    for length, count in sorted(length_counts.items()):
        bar = "*" * int((count / max_count) * column_width)
        # Zarovnání počtu na pravý okraj
        print(f"{length:{max_len_digits}} {bar:>12} ({count:>2})") # Změna formátování výstupu
